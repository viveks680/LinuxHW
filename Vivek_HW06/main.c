...
// Vivek Senapati: Custom code for threads
...
/* Print header */
printk(KERN_INFO "Vivek Senapati: %-*s %3s %5s %2s %-5s %-*s %8s %s\n",
...
uid[kUidLen] = '\0';  /* Guarentee uid is null terminated. */
  	    ...
  	    cpu = runtime / (ktime_get_raw_ns() - tsk->start_time);
  	    ...
  	    printk(KERN_INFO "Vivek Senapati: "
...
#define my_kthread_create(function, data, clone) ({                       \
  	    pid_t mypid;                                                          \
  	    printk(KERN_INFO "Vivek Senapati: Calling kthread(" #function ")\n"); \
  	    mypid = kernel_thread((function), data, clone);                       \
  	    printk(KERN_INFO "Vivek Senapati: " #function " pid = %d\n", mypid);  \
  	    mypid;                                                                \
  	})
...
#define declare_my_kthread_do(task_name)                                  \
 	  static int my_kthread_do_##task_name(void *unused) {                    \
 	    set_task_comm(current, "Vivek Senapati: " #task_name);                            \
 	    set_current_state(TASK_RUNNING);                                      \
 	    printk(KERN_INFO "Vivek Senapati: " #task_name " is about to "        \
 	                     "schedule.\n");                                      \
 	    schedule();                                                           \
 	    printk(KERN_INFO "Vivek Senapati: " #task_name " has been "           \
 	                     "scheduled.\n");                                     \
 	    do_exit(0);                                                           \
 	    return 0;                                                             \
 	  }
 ...
 DEFINE_PER_CPU(struct task_struct *, my_hotplugd);
	...
	static void my_hotplugd_create(unsigned cpu) {
	  printk(KERN_INFO "Vivek Senapati: hotplugd is up on CPU %u.\n", cpu);
	}
	static void run_my_hotplugd(unsigned cpu) {
	  printk(KERN_INFO "Vivek Senapati: hotplugd is running on CPU %u.\n", cpu);
 ...
 .thread_should_run  = should_my_hotplugd_run,
  ...
  .thread_comm        = "Vivek Senapati: hotplugd/%u",
	};
...
static int __ref kernel_init(void *unused)
{
     ...
     */
  	  ...
  	  printk(KERN_INFO "Vivek Senapati: my_kthread_do* are about to be created.\n");
  	  my_kthread_create(my_kthread_do_task_1, NULL, CLONE_FS);
     my_kthread_create(my_kthread_do_task_2, NULL, CLONE_FS);
  	  printk(KERN_INFO "Vivek Senapati: my_kthread_do* have been created.\n");
